# step10_xgboost.py (Final, Ultra-Compact CUDA Version - Enhanced)
...
# (No changes needed in the kernel itself)
...
__device__ float atomicMinFloat(float* addr, float value) {
    ...
    unsigned int new_val_uint = __float_as_int(value);
    while (__int_as_float(old_val_uint) > value) {
        unsigned int assumed_val_uint = old_val_uint;
        old_val_uint = atomicCAS(addr_as_uint, assumed_val_uint, new_val_uint);
        if (assumed_val_uint == old_val_uint) break;
    }
    return __int_as_float(old_val_uint);
}
...
void find_best_split_mse(const float* data, const float* residuals, const int* sample_indices,
    ...
    if (sample_to_test_idx < n_samples_in_node) {
        ...
        if (n_left > 0 && n_right > 0) {
            ...
            if (weighted_mse < atomicMinFloat(min_mse, weighted_mse)) {
...
# CHANGED: Increased n_estimators for a more powerful model.
# ADDED: Subsampling parameters for regularization and speed.
n_estimators = 150
lr = 0.1
...
for i in range(n_classes):
    ...
    for _ in range(n_estimators):
        ...
        # ADDED: Implement subsampling of data and features for this iteration
        n_samples_subset = int(subsample * n_samples)
        d_sample_idx = cp.random.choice(n_samples, n_samples_subset, replace=False).astype(cp.int32)

        n_features_subset = int(colsample_bytree * n_features)
        d_feature_idx = cp.random.choice(n_features, n_features_subset, replace=False).astype(cp.int32)
        
        d_min_mse, d_best_feat, d_best_thresh = cp.array([1e9],'f4'), cp.array([-1],'i4'), cp.array([-1.],'f4')
        
        # Launch kernel with the subsets of data and features
        tpb, bpg = (256,), ((n_features_subset * n_samples_subset + 255) // 256,)
        find_best_split_mse_kernel(bpg, tpb, (d_X_train, d_residuals, d_sample_idx, n_samples_subset,
            n_features, d_feature_idx, n_features_subset, d_min_mse, d_best_feat, d_best_thresh))
        
        # CHANGED: Use .item() to fix DeprecationWarning and get a clean scalar
        best_feat, best_thresh = int(d_best_feat.item()), float(d_best_thresh.item())
        
        if best_feat != -1:
            d_left_mask = d_X_train[:, best_feat] <= best_thresh
            
            # Ensure masks are not empty to avoid division by zero
            if cp.any(d_left_mask) and cp.any(~d_left_mask):
